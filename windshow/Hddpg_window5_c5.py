...
agent.worker_network.load_state_dict(torch.load(log_dir_ddpg_5))
...
def task_fn_H():
    env = PPortfolioEnv(df=df_train, steps=steps, window_length=window, output_mode='EIIE',
...
def task_fn_test_H_5():
    env = PPortfolioEnv(df=df_test, steps=500, window_length=window, output_mode='EIIE',
                        gamma=2, c=0.045, trading_cost=0.0025, utility='Log', scale=True,
                        scale_extra_cols=True, random_reset=False)
    env = RobTransposeHistory(env)
    env = RobConcatStates(env)
    env = RobSoftmaxActions(env)
...
def task_fn_test_H_13():
    env = PPortfolioEnv(df=df_test, steps=500, window_length=window, output_mode='EIIE',
                        gamma=2, c=0.055, trading_cost=0.0025, utility='Log', scale=True,
                        scale_extra_cols=True, random_reset=False)
    env = RobTransposeHistory(env)
    env = RobConcatStates(env)
    env = RobSoftmaxActions(env)
