...
def task_fn_test():
    env = PortfolioEnv(df=df_test, steps=650, window_length=window, output_mode='EIIE',
                       trading_cost=0.0025, utility='Log', scale=True, scale_extra_cols=True,
                       include_cash=True)
    env = TransposeHistory(env)
    env = ConcatStates(env)
...
class DDPGAgent(mp.Process):
    ...
    def episode(self, deterministic=False, video_recorder=None):
        ...
        while True:
            ...
            next_state *= config.reward_scaling
            ...
            log_value(prefix + 'total_reward', total_reward, self.total_steps)
...
class DeterministicCriticNet(nn.Module, BasicNet):
    def __init__(self,
        ...
        self.layer3 = nn.Linear((h1 + 2) * action_dim, 1)
        self.non_linear = non_linear
        # self.layer3.weight.data.uniform_(-0.01, 0.01)
        if batch_norm:
    ...
    def forward(self, x, action):
        ...
        # h = self.non_linear(h)
        batch_size = x.size()[0]
        action = self.layer3(h.view((batch_size, -1))) #TODO modify here
        return action
...
config.actor_network_fn = lambda: DeterministicActorNet(
    task.state_dim, task.action_dim, action_gate=False, action_scale=1.0, non_linear=F.relu,
...
config.random_process_fn = lambda: OrnsteinUhlenbeckProcess(size=task.action_dim, theta=0.3, sigma=0.4,
                                                            sigma_min=0.00002, n_steps_annealing=10000)

config.discount = 0.9
config.min_memory_size = 500
...
config.save_interval = config.episode_limit = 50000
