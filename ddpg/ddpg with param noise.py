...
from component.replay_buffer import ReplayMem, Transition
...
class DDPGAgent:
    def __init__(self, config):
        ...
        self.param_noise = AdaptiveParamNoiseSpec(initial_stddev=0.1, desired_action_stddev=0.3, adaptation_coefficient=1.05)
    ...
    def perturb_actor_parameters(self, param_noise):
        ...
        self.hard_update(self.actor_perturbed, self.worker_network.actor)
    ...
    def episode(self, deterministic=False, video_recorder=None):
        ...
        self.perturb_actor_parameters(param_noise = self.param_noise)
        ...
        while True:
            ...
            #if not deterministic:  # TODO normal OU process
                #action += self.random_process.sample()
            if self.param_noise is not None:
                action = self.actor_perturbed.predict(np.stack([state])).flatten()
            else:
                action = actor.predict(np.stack([state])).flatten()
                action += self.random_process.sample()
                #actor.train()
            ...
            # update param_noise
                #noise_counter += 1
                #if self.replay.pos - noise_counter >= 0:
                #    episode_transitions = self.replay.sample()[self.replay.pos-noise_counter:self.replay.pos]
                #else:
                #    episode_transitions = self.replay.sample()[self.replay.pos - noise_counter + 100000:100000] \
                #                 + self.replay.sample()[0:self.replay.pos]
        ...
        # update param_noise
        if self.replay.pos - noise_counter >= 0:
            noise_states = self.replay.states[self.replay.pos-noise_counter:self.replay.pos]
            noise_actions = self.replay.actions[self.replay.pos-noise_counter:self.replay.pos]
        else:
            noise_states = self.replay.states[0:self.replay.pos]
            noise_actions = self.replay.actions[0:self.replay.pos]
        #noise_states, noise_actions, noise_rewards, noise_next_states, noise_terminals = noise_data
        unperturbed_actions = actor.predict(noise_states)
        perturbed_actions = noise_actions
...
#config.replay_fn = lambda: ReplayMem(capacity=int(1e9))
