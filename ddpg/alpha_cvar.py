...
def task_fn():
    env = PortfolioEnv(df=df_train, steps=128, window_length=window, utility='Log', output_mode='EIIE', gamma=1,
                       scale= True, trading_cost=0.0025, scale_extra_cols=True)
    ...
    def episode(self, deterministic=False, video_recorder=None):
        ...
        while True:
            ...
            if not deterministic and self.replay.size() >= config.min_memory_size:
                ...
                # qq = Amu - alphas**-1 * norm.pdf(norm.ppf(alphas))*torch.sqrt(Asigma)
...
config.replay_fn = lambda: ReplayAlpha(memory_size=int(1e6), batch_size=32)
...
# log_dir_H = '/Users/Morgans/Desktop/trading_system/video/distributional_ddpg_cvar_win10_30_etf.pth'
# agent.save(log_dir_H)
