...
class DDPGAgent:
    ...
    def soft_update(self, target, src):
        for target_param, param in zip(target.parameters(), src.parameters()):
            ...
            target_param.data.copy_(target_param.data * (1.0 - self.config.target_network_mix) + param.data * self.config.target_network_mix)
...
class DeterministicCriticNet(nn.Module, BasicNet):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(2, h2, (3, 1))
        self.conv2 = nn.Conv2d(h2, h1, (stride_time, 1), stride=(stride_time, 1))
        self.layer3s = nn.Linear((h1+2)*action_dim, h0)
        self.layer4 = nn.Linear(h0, 1)
        # self.conv1a = nn.Conv2d(1, h0, (1, 1))
        # self.layer1a = nn.Linear(action_dim, h2)
        # self.layer2a = nn.Linear(h2, 64)
        # self.conv2a = nn.Conv2d(h0, h1, (1, 1))
        # self.layer_t1 = nn.Linear(64, 32)
        # self.layer_t2 = nn.Linear(32, 1)
        # self.layer4 = nn.Linear(32, 1)
        # self.conv1s.weight.data.uniform_(0, 0.01)
        # nn.init.uniform_(self.layer3.weight, a=-0.003, b=0.003)
        ...
        # self.layer1a.weight.data.uniform_(0, 0.01)
...
class DeterministicActorNet(nn.Module, BasicNet):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(2, h2, (3, 1))  # input 64 * 50 * 10   output 64 *48 *8
        self.conv2 = nn.Conv2d(h2, h1, (stride_time, 1), stride=(stride_time, 1))
...
config.actor_optimizer_fn = lambda params: torch.optim.Adam(params, lr=3e-5)
config.critic_optimizer_fn = lambda params: torch.optim.Adam(params, lr=3e-4, weight_decay=0.01)
...
#agent
#torch.save(agent.worker_network.state_dict(), save_file) # TODO save parameter
#agent2.worker_network.load_state_dict(torch.load('agent1')) # TODO load the paramter
...
log_dir = root + '/video/DDPGAgent-ddpg-agent-ETF-win10—feature2-etf_chn.pth'
...
def test_algo(env, algo):
    ...
    actions = []
    while not done:
        algo.worker_network.actor.eval()
...
class DDeepRLWrapper(gym.Wrapper):
    ...
    def step(self, action1, action2):
        ...
        state = self.normalize_state(state)
        reward_gt *= 1000
        reward_at *= 1000# often reward scaling
...
def task_fn_H():
    env = PPortfolioEnv(df=df_train, steps=256, window_length=window, output_mode='EIIE', gamma = 2, c= 0.05,
                       trading_cost=0.0025, utility='Log', scale=True, scale_extra_cols=True)
...
def task_fn_test_H():
    env = PPortfolioEnv(df=df_test, steps=500, window_length=window, output_mode='EIIE', gamma= 2, c= 0.05,
                       trading_cost=0.0025, utility='Log', scale=True, scale_extra_cols=True)
    env = RobTransposeHistory(env)
    env = RobConcatStates(env)
    env = RobSoftmaxActions(env)
...
def task_fn_vali_H():
    env = PPortfolioEnv(df=df_train, steps=2000, window_length=window, output_mode='EIIE', gamma= 2, c=0.05,
                        trading_cost=0.0025, utility='Log', scale=True, scale_extra_cols=True)
    env = RobTransposeHistory(env)
    env = RobConcatStates(env)
    env = RobSoftmaxActions(env)
...
class Hierachicalagent(nn.Module, BasicNet):
    ...
    def forward(self, x, w1):
        ...
        w0 = x[:, :1, :1, :]  # weights from last step
        x = x[:, :, 1:, :]   # remove a_t
        w1 = self.to_torch_variable(w1)[:, None, None, :]
        phi0 = self.non_linear(self.conv0(x))
        if self.batch_norm:
            phi0 = self.bn1(phi0)
...
class Critichierachical(nn.Module, BasicNet):
    def __init__(self,
        ...
        self.conv2 = nn.Conv2d(h2, h1, (stride_time, 1), stride=(stride_time, 1))
        self.layer3 = nn.Linear((h1 + 2) * action_dim, 1) # TODO adding action2
        self.non_linear = non_linear
    ...
    def forward(self, x, action1, action2):
        x = self.to_torch_variable(x)
        action1 = self.to_torch_variable(action1)[:, None, None, :]  # remove cash bias
        action2 = self.to_torch_variable(action2)[:, None, None, :]
        w0 = x[:, :1, :1, :]
...
config1.actor_optimizer = lambda params: torch.optim.Adam(params, lr=5e-5) # weight_decay=1e-4)
config1.critic_optimizer = lambda params: torch.optim.Adam(params, lr=5e-4, weight_decay=0.01)
# config.replay_fn = lambda: ReplayMemory(capacity=int(1e9))
# config.replay_fn = lambda: HighDimActionReplay(memory_size=int(1e6), batch_size=32)
config1.random_process_fn = lambda: OrnsteinUhlenbeckProcess(size=task_H.action_dim, theta=0.3, sigma=0.3, sigma_min=0.002, n_steps_annealing=10000)
config1.replay_Hierachical = lambda: RobHighDimActionReplay(memory_size=int(1e6), batch_size=64)
config1.discount = 0.99
config1.min_memory_size = 500
...
# log_dir_H = '/Users/Morgans/Desktop/trading_system/video/HiAgentddpg_sharp.pth'
# log_dir_H = root+ '/video/HiAgentddpg_sharp.pth'
