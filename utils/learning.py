...
def learning(agent):
    ...
    while True:
        ...
        if config.save_interval and ep % config.save_interval == 0:
            with open('/Users/Morgans/Desktop/trading_system/video/%s-%s-online-stats-%s.bin' % (
                    agent_type, config.tag, agent.task.name), 'wb') as f: pickle.dump([steps, rewards], f)
        ...
        if config.test_interval and ep % config.test_interval == 0:
            ...
            agent.save('/Users/Morgans/Desktop/trading_system/video/%s-%s-model-%s.bin' % (agent_type, config.tag, agent.task.name))
            test_rewards = []
            for _ in range(config.test_repetitions):
                test_rewards.append(agent.episode(True)[0])
            avg_reward = np.mean(test_rewards)
            avg_test_rewards.append(avg_reward)
            config.logger.info('Avg reward %f(%f)' % (avg_reward, np.std(test_rewards) / np.sqrt(config.test_repetitions)))
            with open('/Users/Morgans/Desktop/trading_system/video/%s-%s-all-stats-%s.bin' % (agent_type, config.tag, agent.task.name), 'wb') as f:
                pickle.dump({'rewards': rewards, 'steps': steps, 'test_rewards': avg_test_rewards}, f)

        # update the parem_noise
