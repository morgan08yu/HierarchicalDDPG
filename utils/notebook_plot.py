...
class LivePlotNotebook(object):
    ...
    def __init__(self, log_dir=None, episode=0, labels=[], title='', ylabel='returns', colors=None, linestyles=None, legend_outside=True):
        if  matplotlib.rcParams['backend'] == 'nbAgg': # TODO remove not
            logging.warning("The liveplot callback only work when matplotlib is using the nbAgg backend. Execute 'matplotlib.use('nbAgg', force=True)'' or '%matplotlib notebook'")
