...
class PortfolioSim(object):
    ...
    def __init__(self, steps, asset_names=[], trading_cost=0.0025, time_cost=0.0000, utility='Log', gamma=1, alpha=0.05):
    ...
    def assets_historical_returns_and_covariances(self, prices):
        ...
        returns = (1 + expreturns) ** 250 - 1  # Annualize returns
        vars = covars * 250  # Annualize covariances
    ...
    def _step(self, w1, y1, cprice):
        ...
        if self.utility == 'Log':  # log rate of return
            r1 = np.log(p1 / p0)  # - y1.mean()  #np.log
...
class PortfolioEnv(gym.Env):
    ...
    def plot_notebook(self, close):
        ...
        if not self._plot4:
            self._plot_dir4 = os.path.join(self.log_dir,
                                           'notebook_plot_cost_' + str(time.time())) if self.log_dir else None
            self._plot4 = LivePlotNotebook(log_dir=self._plot_dir4, labels=['portfolio risk']+['market risk'], title='risk', ylabel='risk')
